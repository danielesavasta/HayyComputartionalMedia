::: swiper-slide cover

# New Media and Generative Thinking

#### Phase 2

#### Daniele Savasta / Ege Canpolat

##### Hayy Açık Okul

:::

::: swiper-slide

# Generative Creativity

The artist designs a system and allows that system to produce outcomes with a degree of autonomy. Not controlling the result but designing the conditions.

Galanter's definition: the author creates a system and cedes a degree of control to it. Directly connected to Manovich's principle of automation: when media becomes numerical, it becomes open to rule-based processes that operate with partial independence from the author.

:::

::: swiper-slide

## Common Anatomy: Input / Process / Output

<svg class="flow-svg" viewBox="0 0 760 185" xmlns="http://www.w3.org/2000/svg">
<text x="40" y="76" text-anchor="middle" font-family="sans-serif" font-size="11" font-weight="700" letter-spacing="2" fill="currentColor" opacity="0.4">WORLD</text>
<line x1="68" y1="73" x2="116" y2="73" stroke="currentColor" stroke-width="1.5"/>
<polygon points="116,68 126,73 116,78" fill="currentColor"/>
<rect x="128" y="48" width="110" height="50" rx="3" fill="none" stroke="currentColor" stroke-width="2"/>
<text x="183" y="78" text-anchor="middle" font-family="sans-serif" font-size="13" font-weight="700" letter-spacing="2" fill="currentColor">INPUT</text>
<line x1="240" y1="73" x2="286" y2="73" stroke="currentColor" stroke-width="1.5"/>
<polygon points="286,68 296,73 286,78" fill="currentColor"/>
<rect x="298" y="48" width="135" height="50" rx="3" fill="none" stroke="currentColor" stroke-width="2"/>
<text x="365" y="78" text-anchor="middle" font-family="sans-serif" font-size="13" font-weight="700" letter-spacing="2" fill="currentColor">PROCESS</text>
<line x1="435" y1="73" x2="481" y2="73" stroke="currentColor" stroke-width="1.5"/>
<polygon points="481,68 491,73 481,78" fill="currentColor"/>
<rect x="493" y="48" width="115" height="50" rx="3" fill="none" stroke="currentColor" stroke-width="2"/>
<text x="550" y="78" text-anchor="middle" font-family="sans-serif" font-size="13" font-weight="700" letter-spacing="2" fill="currentColor">OUTPUT</text>
<line x1="610" y1="73" x2="656" y2="73" stroke="currentColor" stroke-width="1.5"/>
<polygon points="656,68 666,73 656,78" fill="currentColor"/>
<text x="702" y="76" text-anchor="middle" font-family="sans-serif" font-size="11" font-weight="700" letter-spacing="2" fill="currentColor" opacity="0.4">WORLD</text>
<path d="M 606 98 L 606 148 L 130 148 L 130 98" fill="none" stroke="currentColor" stroke-width="1.5" stroke-dasharray="5,4" opacity="0.55"/>
<polygon points="125,100 130,90 135,100" fill="currentColor" opacity="0.55"/>
<text x="368" y="163" text-anchor="middle" font-family="sans-serif" font-size="10" font-weight="700" letter-spacing="3" fill="currentColor" opacity="0.45">FEEDBACK</text>
</svg>

**Input:** what the world gives the system: human presence, sensor data, image, sound.

**Process:** the rule the system applies: code, model, node-based flow, microcontroller logic.

**Output:** what the system gives back: light, sound, image, movement, heat, smoke.

**Feedback:** output becoming input again. The system watches itself and adjusts.

:::

::: swiper-slide

<div class="map-slide">
  <h3 class="map-title">Generative Creation Map</h3>

  <div class="map-entry-row">
    <div class="map-node entry" onclick="swiperInstance.slideTo(4,400)">Media Anatomy</div>
  </div>
  
  <div class="map-row">
    <div class="map-label">Foundation<br>Principles</div>
    <div class="map-cards">
      <div class="map-node" onclick="swiperInstance.slideTo(5,400)">Numerical Representation</div>
      <div class="map-node" onclick="swiperInstance.slideTo(6,400)">Procedurality</div>
      <div class="map-node" onclick="swiperInstance.slideTo(7,400)">Crossing Senses / Intermediality</div>
      <div class="map-node" onclick="swiperInstance.slideTo(8,400)">Co-existence</div>
    </div>
  </div>
  
  <div class="map-row">
    <div class="map-label">Properties</div>
    <div class="map-cards">
      <div class="map-node" onclick="swiperInstance.slideTo(9,400)">Generativity / Variation</div>
      <div class="map-node" onclick="swiperInstance.slideTo(10,400)">Modularity / Repetition</div>
      <div class="map-node" onclick="swiperInstance.slideTo(11,400)">Connectivity / Network</div>
      <div class="map-node" onclick="swiperInstance.slideTo(12,400)">Limits</div>
    </div>
  </div>
  
  <div class="map-row">
    <div class="map-label">Process</div>
    <div class="map-cards">
      <div class="map-node process" onclick="swiperInstance.slideTo(13,400)">Profanation / Appropriation</div>
      <div class="map-node process" onclick="swiperInstance.slideTo(14,400)">Alea / Abandon</div>
      <div class="map-node process" onclick="swiperInstance.slideTo(15,400)">Randomness / Chaos / Accident / Entropy</div>
    </div>
  </div>
  
  <div class="map-row">
    <div class="map-label">Meaning<br>Making</div>
    <div class="map-cards">
      <div class="map-node" onclick="swiperInstance.slideTo(16,400)">Text as Meaning / Poetic Computation</div>
      <div class="map-node" onclick="swiperInstance.slideTo(17,400)">Interaction Ecology</div>
      <div class="map-node" onclick="swiperInstance.slideTo(18,400)">Lore</div>
    </div>
  </div>
  
  <div class="map-row">
    <div class="map-label">Tools</div>
    <div class="map-cards">
      <div class="map-node" onclick="swiperInstance.slideTo(19,400)">p5.js / Processing</div>
      <div class="map-node" onclick="swiperInstance.slideTo(20,400)">Sensors / Hardware / Software / Network</div>
    </div>
  </div>
</div>

:::

::: swiper-slide
# Media Anatomy
Every digital media object (image, sound, video, interactive system) shares the same underlying structure. Understanding that structure is the first step toward working with it intentionally.

:::

::: swiper-slide
## Foundation Principles
# Numerical Representation
Every media object consists of digital code: mathematically definable, open to algorithmic manipulation. Media becomes programmable.

A photograph is a grid of numbers. A sound is a sequence of values. This numerical nature means any media object can be transformed, generated, or combined algorithmically.

:::

::: swiper-slide
<iframe src="http://dada.compart-bremen.de/item/agent/15"></iframe>
:::

::: swiper-slide
## Foundation Principles
# Procedurality
The procedure (the algorithm, the instruction set) is often the artwork itself, not just the means of making it.

You write the rules; the rules write the world. The artist is the author of conditions, not of outcomes.
:::

::: swiper-slide
<iframe src="https://ropac.net/artists/231-vera-molnar/"></iframe>
Vera Molnár
:::

::: swiper-slide
## Foundation Principles
# Crossing Senses / Intermediality
The same data moves between mediums: sound becoming light, movement becoming image, text becoming space.

Media is not locked to one sense or form. Intermediality is letting one data source speak in multiple languages at once.
:::

::: swiper-slide
<iframe src="https://www.children-of-the-light.com/projects/all-together-now/4a108ccb-c4f7-46d6-acd8-5868a15fdf03"></iframe>
:::

::: swiper-slide
<iframe src="https://www.borisacket.nl/"></iframe>
:::

::: swiper-slide
## Foundation Principles
# Co-existence

The simultaneous presence of analog and digital, physical and virtual, human and nonhuman within the same system or space.

Co-existence is about designing the relationship between these registers: how they touch, depend on, and transform each other.
:::

::: swiper-slide
<iframe src="https://ka5.digital/projects/prospectus-for-a-future-body"></iframe>

### Choy Ka Fai, Prospectus for a Future Body. 2010
:::

::: swiper-slide
<video controls>
  <source src="videos/mimu_imogenHeap.mp4" type="video/mp4" title="mimu_imogenHeap">
</video>

### Imogen Heap, Mimu Glove. 2010
:::

::: swiper-slide
## Properties
# Generativity / Variation
How changing one variable produces an entire family of related but distinct forms, sounds, or behaviors.

The artist designs the conditions; the system generates the instances. Variation is controlled difference within a defined space of possibility.
:::

::: swiper-slide
<iframe src="https://www.tylerxhobbs.com/words/fidenza"></iframe>

### Tyler Hobbs, Fidenza. 2021
:::


::: swiper-slide
## Properties
# Modularity / Repetition
A small repeating element, when scaled and layered, becomes something structurally new.

The module retains its identity while contributing to a larger whole. At scale, repetition produces emergence. What was one thing becomes a field, a texture, an environment.
:::

::: swiper-slide
<video controls>
  <source src="videos/zimoun.mp4" type="video/mp4" title="zimoun">
</video>

### Zimoun, Sound Installations & Sound Sculptures. 2025
:::

::: swiper-slide
<video controls>
  <source src="videos/danielRozin_ponponMirror.mp4" type="video/mp4">
</video>

### Daniel Rozin, Pon Pon Mirror. 2015
:::

::: swiper-slide
<video controls>
  <source src="videos/.mp4" type="video/mp4">
</video>

### Jussi Ängeslevä, ART+COM, Olafur Arnalds, MKT AG, RGB|CMY Kinetic. 2015
:::

::: swiper-slide
## Properties
# Connectivity / Network
How devices, sensors, spaces, and people share information and collectively respond to the world.

A connected system is not the sum of its parts. It is the sum of its relationships. New behaviors emerge that no single component could produce alone.
:::

::: swiper-slide
<video controls>
  <source src="videos/looksLikeMusicYuriSuzuki.mp4" type="video/mp4">
</video>

### Yuri Suzuki, Looks Like Music. 2013
:::


::: swiper-slide
## Properties
# Limits
Origins. Constraints. Dataset. Pool. Source.

Every system is defined as much by what it cannot do as by what it can. Limits are not obstacles. They are the edges that give the work its character.
:::

::: swiper-slide
## Process
# Profanation / Appropriation
Taking something from one context and returning it to common use in a different one. Not destruction. Displacement.

The same material, made to mean something else.
:::

::: swiper-slide
<video controls>
  <source src="videos/GraffitiResearchLabs_ledThrowies.mp4" type="video/mp4">
</video>

### Graffiti Research Lab, Led Throwies. 2006
:::

::: swiper-slide
<video controls>
  <source src="videos/golanLevin_augmentedHand.mp4" type="video/mp4">
</video>

### Golan Levin, Chris Sugrue, and Kyle McDonald, Augmented Hand Series. 2014
:::

::: swiper-slide
## Process
# Alea / Abandon
The generative process produces more than you intend. At some point: intervene and edit, or abandon and let the system run again.

This is not failure. It is the practice. The decision defines the relationship between the artist and the system.
:::

::: swiper-slide
## Process
# Randomness / Chaos / Accident / Serendipity / Letting Go / Entropy
The system produces things you did not plan. Some are more interesting than anything you could have planned.

Accident is not the enemy of intention. Letting go is the practice of remaining open to what the system discovers on its own.
:::

::: swiper-slide
<video controls>
  <source src="videos/concave_MehmetBerkBostancı.mp4" type="video/mp4">
</video>

### Mehmet Berk Bostancı, Concave. 2024
:::

::: swiper-slide
<iframe src="https://mehmetberkbostanci.com/portfolio/concave/"></iframe>

### Mehmet Berk Bostancı, Concave. 2024
:::

::: swiper-slide
## Meaning Making
# Text as Meaning / Poetic Computation
Creative coding. Vibe-coding. LLMs.

Language can be the material the work is made of, not just a description of it. Code is writing. Prompts are composition. Computation becomes a poetic act.
:::

::: swiper-slide
<video controls>
  <source src="videos/BagerAkbaŞairDenizYılmaz.mp4" type="video/mp4">
</video>

### Bager Akbay, Şair Deniz Yılmaz. 2024
:::


::: swiper-slide
## Meaning Making
# Interaction Ecology
How human-nonhuman-machines interact within a frame or space, and how their roles alter in relation to each other.

Not a single user pressing a button. A field of bodies, sensors, outputs, and histories in simultaneous relation.
:::

::: swiper-slide
<video controls>
  <source src="videos/theoWatson_ConnectedWorlds.mp4" type="video/mp4">
</video>

### Studio IO, Theo Watson, Connected Worlds. 2016
:::

::: swiper-slide
## Meaning Making
# Lore
Context. Artistic Intention.

What separates "a grid of blinking LEDs" from "a field of breathing organisms." The references a piece carries, the world it implies, the intention that makes formal choices feel deliberate rather than arbitrary.
:::

::: swiper-slide
<video controls>
  <source src="videos/vanGogh_nohLab.mp4" type="video/mp4">
</video>

### Nohlab, VAN GOGH: AN IMMERSIVE JOURNEY. 2022
:::

::: swiper-slide
<video controls>
  <source src="videos/ChrisMilkTheTreacheryofSanctuary.mp4" type="video/mp4">
</video>

### Chris Milk, The Treachery of Sanctuary. 2014
:::

::: swiper-slide
<video controls>
  <source src="videos/lesmétamorphosesdeMrKalia.mp4" type="video/mp4">
</video>

### Béatrice Lartigue & Cyril Diagne, Les métamorphoses de Mr Kalia. 2014
:::

::: swiper-slide
<video controls>
  <source src="videos/menstruationMachine.mp4" type="video/mp4">
</video>

### Sputniko!, Menstruation Machine. 2010
:::

::: swiper-slide
##### Tools
## p5.js / Processing
Code-based environments for visual, sonic, and interactive work. Both treat the screen as a sketchbook for rapid iteration.

You write a rule, you see what it makes. The most direct path from an algorithmic idea to a visible result.
:::

::: swiper-slide
##### Tools
## Sensors / Hardware / Software / Network
Microcontrollers, sensors, actuators, and the protocols that connect them: OSC, MQTT, WebSocket.

This is where the system touches the world. A sensor reads the environment; software decides; an actuator acts.
:::

::: swiper-slide
<video controls>
  <source src="videos/ChrisMilkTheTreacheryofSanctuary.mp4" type="video/mp4">
</video>

### Chris Milk, The Treachery of Sanctuary. 2014
:::


::: swiper-slide .references
 - https://csugrue.com/
 - https://kylemcdonald.net/
 - https://flong.com/archive/index.html
 - https://joshuadavis.com/
 - https://roberthodgin.com/
 - https://quasimondo.com/
 - https://studiodumbar.com/
 - https://www.johngerrard.net/
 - https://cyrildiagne.com/
 - https://beatricelartigue.com/
 - https://www.evan-roth.com/timeline/
 - http://milk.co/
 - http://zach.li/
 - https://reas.com/
 - https://www.benfry.com/
 - https://nohlab.com/
 - https://link.bagerakbay.com/
:::

::: swiper-slide

## Keywords

Authorship, Cession, Multiplicity, Alea, Procedurality, Cohabitation, Profanation, Emergence, Corpus, Numericity, Abandonment, Displacement, Autonomy, Instruction, Possibility space, Serendipity, Choreography, Negotiation, Skin, Gesture, Lore, Glitch, Interoperability, Feedback, Topology, Intermediality, Notation, Diffusion, Threshold, Nonhuman, Actuator, Infestation, Register, Inscription, Ecology, Drift, Contingency, Rewilding, Artifact, Transmediality, Instinct, Protocol, Flocking, Attractor, Mediation, Self-organization, Apparatus, Leakage, Camouflage, Residue, Terrain, Phantom, Modulation, Rule, Semiosis, Stochasticity, Porosity, Sympoiesis, Erosion, Recursion, Relay, Incompleteness, Materiality, Intersubjectivity, Scaffold, Decay, Propagation, Underdetermination, Contamination, Aleatory, Hauntology, Palimpsest, Morphogenesis, Trace, Archive, Membrane, Enactivism, Noise, Situated, Hybridization, Uncanny, Overgrowth, Embodiment, Distributed, Becoming, Performativity, Signal, Indeterminacy, Coexistence, Mutation, Mesh, Rhizome, Dissipation, Spectrality, Intertextuality, Ergodic, Fragmentation, Oscillation, Sediment

:::